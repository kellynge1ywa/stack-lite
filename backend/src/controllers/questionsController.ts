...
import {questionsValidationSchema} from '../helpers/questionsValidation'
...
export const addQuestion=async (req:ExtendedRequest,res:Response)=>{
    try {
        ...
        const {error}=questionsValidationSchema.validate(req.body)
        if(error){
        ...
        .input('Tags',Tags)
        .input('Body',Body)
        .execute('add_question_procedure')
